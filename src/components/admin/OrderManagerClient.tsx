...
import { Search, X, Save, ChevronLeft, ChevronRight } from "lucide-react";
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="bg-white flex-1 p-3 rounded-2xl shadow-sm border border-slate-100 flex items-center gap-3">
            ...
            <input
              ...
              className="flex-1 bg-transparent border-none focus:outline-none text-slate-700 font-medium text-sm"
            />
          </div>
          <select
            value={statusFilter}
            onChange={e => handleFilterChange(e.target.value)}
            className="bg-white p-3 rounded-2xl border border-slate-100 shadow-sm text-sm font-bold text-slate-700 focus:outline-none"
          >
              ...
              <tbody className="divide-y divide-slate-100">
                {paginated.map(order => (
                  <tr key={order.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="p-4 font-bold text-slate-700 text-sm">{order.planName}</td>
                    <td className="p-4">
                      <span className={`px-2.5 py-1 text-[10px] font-bold rounded-full border ${statusColors[order.status] || "bg-slate-100 text-slate-500 border-slate-200"}`}>
                        {order.status}
                      </span>
                    ...
                    <td className="p-4 text-right">
                      <select
                        ...
                        className="text-xs font-bold bg-slate-100 text-slate-700 py-2 px-3 rounded-lg border border-slate-200 cursor-pointer hover:bg-slate-200 focus:outline-none disabled:opacity-50"
                      >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white w-full max-w-sm rounded-2xl p-6 shadow-2xl mx-4">
            ...
            <input
              ...
              className="w-full border-2 border-slate-200 rounded-xl px-4 py-3 text-sm font-medium focus:border-emerald-500 focus:outline-none mb-4"
            />
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex-1 py-3 bg-emerald-600 text-white font-bold rounded-xl hover:bg-emerald-700 disabled:opacity-50 flex items-center justify-center gap-2 text-sm"
              >
